...
# Neutralize mobile stacking so the phone view matches the laptop
# view exactly (combined with the width=1280 viewport meta injected
# from app_shell.inject_css).
# ─────────────────────────────────────────────────────────────────
_FORCE_DESKTOP_CSS = """
/* Keep Streamlit columns side-by-side on every width */
...
/* Never collapse the app to a mobile single-column shell */
...
/* Header row: don't wrap on narrow viewports */
.block-container:has(.ui-hdr-anchor) div[data-testid="stHorizontalBlock"]:has(.ui-brand) {
    flex-wrap: nowrap !important;
    height: 64px !important;
    max-height: 64px !important;
}
.ui-wordmark { display: inline-block !important; font-size: 18px !important; }
.ui-mobile-wordmark { display: none !important; }

/* Hero card: keep the desktop two-column grid */
.ui-hero {
    grid-template-columns: 1.2fr 1fr !important;
    padding: 22px 24px !important;
    min-height: 220px !important;
}
.ui-hero-title { font-size: 26px !important; white-space: normal !important; }
.ui-hero-lede  { font-size: 14px !important; }
"""


def _force_desktop() -> str:
    return _FORCE_DESKTOP_CSS


# Wrap build_global_css so the override is always appended last.
_original_build_global_css = build_global_css  # type: ignore[has-type]


def build_global_css(*, login: bool = False) -> str:  # type: ignore[no-redef]
    base = _original_build_global_css(login=login)
    # Insert the override just before the closing </style> so it wins.
    if base.endswith("</style>"):
        return base[:-len("</style>")] + "\n" + _FORCE_DESKTOP_CSS + "\n</style>"
    return base + "<style>\n" + _FORCE_DESKTOP_CSS + "\n</style>"
